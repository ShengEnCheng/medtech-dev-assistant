...
## 一之一、AI 檢索詞抽取（第一關：長文 → 候選詞）

產品說明常是一整段中文計畫書文字。工具可將長文交給語言模型，
抽出四組檢索詞，再進入各資料庫檢索。

**服務**：Ollama Cloud（`https://ollama.com/v1`），模型 `deepseek-v4.1-flash`
**設定**：`OLLAMA_API_KEY`。未設定時自動降級為規則式，功能不中斷。

### 架構：LLM 只提候選，不決定答案

抽出的候選一律放進 `llm_candidates`，由實測決定採用哪個：

| 決定方式 | 用於 |
|---|---|
| 相關性驗證（文獻） | device_en、tech_en |
| TFDA 實測命中數（中文） | tfda_zh |
...
LLM 憑空生成 TFDA 中文品名命中率極差（「心臟血管監測器」0 筆）。
改成先撈出資料庫**實際存在的品名**，讓 LLM 從中挑選。

膀胱案挑出「維塔康超音波膀胱掃描儀」= 最正確的既有品名。
**LLM 不可能編造出不存在的品名。**

### 挑選順序：具體度優先，命中數其次

**命中數最多 ≠ 對。** 膀胱案候選池「超音波」200 筆 vs
「膀胱容量測定儀」2 筆 —— 取最多者會挑中超音波 → 競品 573 件，
含富士軟片等無關廠商。泛用詞的命中數天生就高。

### 品名檢索前必須清理

TFDA 品名格式「“品牌” 核心品名 (附註)」含空格。檢索函式按空格
拆詞 OR 連接 → 「未滅菌」單獨命中大量許可證 → 某候選詞回報 200 筆
→ **競品暴增到 14,676 件**。`clean_query_term()` 去品牌去附註去空格。

### 兩層台灣競品

```
精確層：最接近產品的既有許可證（實測常只有 1 件）
同類層：整個品類的市場地景（品類層級的核心詞）
```

**判讀**：精確少、同類多 → 新切入點（有市場基礎但無直接對應品）；
兩層都少 → 全新品類，教育市場成本高。這是育成輔導最實用的一段。
