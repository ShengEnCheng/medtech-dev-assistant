# 醫療產品開發助手（MedTech Dev Assistant）

> 輸入一段產品說明 → 依勾選的模組（疾病負擔／專利／法規／競品）
> 產出商品化評估報告。供育成中心、Biodesign 課程、SPARK 團隊使用。

---

## 這個專案要解決什麼

Biodesign 與 SPARK 在台灣都以醫療器材為主要標的，兩個計畫的共通終點都是**商品化**。
而商品化最常被團隊跳過、也最容易造成後續白做的三件事，對應 Biodesign 教科書的三個章節：

| 教科書章節 | 團隊要回答的問題 | 本專案模組 |
|-----------|---------------|-----------|
| Stage 2.1 疾病狀態 ＋ 2.2 治療選項 | 這病有多嚴重？現在怎麼治？哪裡不夠好？ | **M0 疾病負擔與市場推估** |
| Stage 4.1 Intellectual Property Basics | 這東西被申請過嗎？我能不能自由實施？ | **M1 專利與 FTO 初篩** |
| Stage 4.2 Regulatory Basics | 走 510(k) 還是 PMA？TFDA 第幾級？ | **M2 法規路徑判定** |
| Stage 2.4 Market Analysis ＋ 4.4 Business Models | 市場上有誰？做到什麼程度？ | **M3 競品與市場地景** |

> **M0 為什麼排第一**：Stanford 官方需求簡報的格式是固定的
> 「背景 → 觀察 → 問題 → 市場」，全部建立在疾病負擔上。
> 只做專利／法規／競品，會在最關鍵的需求陳述與市場規模處交白卷。

這些模組不是外加功能，而是把 Biodesign Stage 2 需求篩選評分裡**已經存在的三個維度**
（Technical Feasibility、Regulatory / Reimbursement Risk、Market Potential）
從「AI 文字推論」升級成「外部資料佐證」，再回寫修正分數。

報告最後一節「**待團隊補齊的資訊**」是刻意設計的：明確標出風險矩陣
（紅／黃／綠）、需求標準（must-have）、利害關係人分析等 Biodesign
要求但工具無法代勞的部分。**報告產出不等於評估完成。**

---

## 快速開始

### 1. 建立環境

```bash
cd /Volumes/MACRAID/projects/medtech-dev-assistant
python3 -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
```

需求：Python 3.10+（開發時使用 3.11）。相依套件只有 streamlit 與 pandas，
核心的三模組引擎只用標準函式庫，**沒有外部 API key 需求**。

### 2. 建立 TFDA 本地索引（首次執行一次，約 2 分鐘）

```bash
python -m src.build_tfda_index
```

會下載衛福部食藥署「醫療器材許可證資料集」（約 16 MB、約 15 萬筆），
建立 SQLite + FTS5 全文索引。之後所有台灣競品查詢都是毫秒級、離線可查。

### 3. 啟動介面

```bash
streamlit run app.py
```

瀏覽器開 http://localhost:8501

### 4. 也可以純命令列使用

```bash
python -m src.cli \
  --product "一款智慧型氣管內管固定裝置，用於加護病房插管病人翻身時偵測管子是否滑脫" \
  --modules regulatory,competitor,patent,paper
```

---

## 專案結構

```
medtech-dev-assistant/
├── app.py                    # Streamlit 介面（主要入口）
├── requirements.txt
├── README.md
├── src/
│   ├── __init__.py
│   ├── schema.py             # 資料結構（dataclass）：三個模組的結果物件
│   ├── http_client.py        # 共用 HTTP（含 openFDA 404 處理、退避重試）
│   ├── tfda_index.py         # TFDA 資料集下載、SQLite FTS5 索引、查詢
│   ├── build_tfda_index.py   # 建索引 CLI
│   ├── query_terms.py        # 產品說明 → 檢索詞抽取（中英映射、泛用詞防護）
│   ├── module_burden.py      # M4 疾病負擔與市場推估
│   ├── module_regulatory.py  # M2 法規路徑判定
│   ├── module_competitor.py  # M3 競品與市場地景
│   ├── module_patent.py      # M1 專利與 FTO 初篩
│   ├── module_paper.py       # M5 論文先前技術（非專利文獻）
│   ├── source_papers.py      # 論文來源（Europe PMC/OpenAlex/Crossref/arXiv）
│   ├── feedback.py           # Evidence 回饋引擎 → 三維分數調整
│   ├── report.py             # Markdown 報告產生
│   └── cli.py                # 命令列入口
├── data/                     # 索引與快取（不進版控）
├── reports/                  # 產出報告（不進版控）
└── docs/
    ├── SOURCES.md            # 資料來源與 API 實測紀錄
    ├── ROADMAP.md            # 開發里程碑
    └── DISCLAIMER.md         # 責任界線與免責聲明
```

---

## 資料來源

### 台灣
| 來源 | 用途 | 筆數 |
|------|------|------|
| TFDA 醫材許可證資料集 | 台灣已取證廠商與品名（**本地索引**） | 104,680 |
| PubMed | 疾病負擔文獻（**M0 核心**） | 依檢索詞 |
| 衛福部統計處 | 死因、長照統計（dl- 連結為真實檔） | 依主題 |
| 國健署 | 國民健康訪問調查 | 依主題 |
| 健保署 | 特材給付現況（**需經代理繞道**） | 依頁面 |

### 全球
| 來源 | 用途 | 筆數 |
|------|------|------|
| openFDA UDI | 全球已上市器材、品牌商 | 267,842 |
| openFDA 製造廠登記 | 全球製造廠分布（41 國） | 335,224 |
| openFDA PMA | 美國高風險器材（Class III） | 4,252 |
| openFDA 510(k) | 前導裝置、法規比對 | 14 萬+ |
| openFDA 召回 | 競品品質風險訊號 | 2,617 |
| openFDA 分類 | 法規分類與路徑 | 6 千+ |
| UN Comtrade | 各國醫材**貿易額**（非產品市場規模） | HS 9018，2022 |
| World Bank | 各國醫療支出佔 GDP | 各國年度 |
| WHO GHO | 全球疾病指標（**台灣無資料**） | 依指標 |
| FreePatentsOnline | 專利檢索（**US/EP/WO/JP/DE 五大局**） | 依檢索詞 |
| EPO OPS | 專利申請人分析（需免費 API key） | 依檢索詞 |
| Europe PMC | 論文先前技術（生醫，含預印本） | 依檢索詞 |
| OpenAlex | 論文先前技術（跨領域，**可篩機構**） | 依檢索詞 |
| Crossref | 論文先前技術（含學位論文） | 依檢索詞 |
| arXiv | 論文先前技術（工程預印本） | 依檢索詞 |

### 已知無法使用（勿再嘗試）
| 來源 | 狀況 |
|------|------|
| Google Patents | 2026-09 實測全面 503（含 HTML 首頁），已停用 |
| EPO OPS | HTTP 403，需註冊 API key（免費但需申請） |
| Lens.org | HTTP 401/403，需 API key |
| EUDAMED | Angular SPA，前端路徑無法取得後端 API |
| WIPO PATENTSCOPE | JSF 表單，可取得結果數但無法抓取清單 |
| DEPATISnet | 有 TSPD 反爬保護，需 JS 執行環境 |

詳細端點、實測數據與已知限制見 `docs/SOURCES.md`。

---

## 論文也會破壞專利新穎性（M5）

**專利的先前技術（prior art）不只有專利。** 學術論文、會議論文、
學位論文、預印本同樣會破壞新穎性。這對校園團隊特別關鍵，因為最常見的
致命情況不是「別人先申請」，而是：

> **老師或團隊成員自己已經發表過相關論文。**

### 這個模組回答三個問題

1. **有沒有論文會擋到？** 跨四個來源檢索，含預印本標記與發表日期
2. **是不是我們自己發的？** 用 ROR 機構碼查長庚體系論文（自我碰撞檢測）
3. **還剩多少時間可以補救？** 各國優惠期對照表

### 各國優惠期差異極大 —— 這是最容易踩的坑

「優惠期多長」和「學術發表算不算」是**兩個不同的問題**：

| 地區 | 優惠期 | 涵蓋學術發表 | 風險 |
|------|-------|------------|------|
| 台灣 | 12 個月 | 是 | 低 |
| 美國 | 12 個月 | 是 | 低 |
| 日本／韓國 | 12 個月 | 是（有程序要求） | 中 |
| 中國 | 6 個月 | **僅限指定學術會議** | 高 |
| 歐洲 (EPO) | 6 個月 | **否** | 極高 |

**歐洲是最大風險區**：EPC Art. 55 的 6 個月優惠期僅限「明顯濫用」
與「官方國際展覽」，**學術發表完全不在適用範圍**。論文一旦公開，
歐洲新穎性即喪失，且無補救方式。

**中國要注意**：專利法第 24 條僅限「規定的學術會議或技術會議」
首次發表，**一般期刊論文發表不適用**。

> 台灣的優惠期是 **12 個月**（專利法第 22 條第 3 項）。
> 網路資料常見「6 個月」是 2022 年修法前的舊資訊。

### 預印本的時間陷阱

arXiv、bioRxiv、SSRN 等預印本，**上線日就是公開日**，
不是正式刊登日。通常比正式發表早 6-12 個月，
用正式發表日計算優惠期會算錯。

### 建議順序

**申請在先、發表在後**是唯一在各國都安全的順序。

### 指定作者或學校查詢

自動檢索只涵蓋長庚體系。若團隊成員來自其他學校或業界，
論文頁籤提供兩種手動查詢：

**依作者姓名**
輸入英文姓名（搭配機構更準），系統列出候選人讓您挑選，再查其著作。

> **為什麼要人工挑選**：台灣姓名英譯重複率極高。實測搜尋
> "Yung-Chun Chang"，候選中混有 Yee-Chun Chen、C.M. Wang 等
> 非同一人。若工具自行認定，會產出錯誤的「己方先前技術」清單。
> 因此一律列出候選（附機構、著作數、h 指數、ORCID）由人判斷。

**依學校**
多選學校（內建 38 所台灣大學與醫學中心），查各校在該主題上的論文。

> 內建清單可擴充：`taiwan_institutions(dynamic=True)` 會向
> OpenAlex 反查全部台灣機構（實測 520 個）並快取。

**公開日的取法**：同一篇論文常有預印本、Zenodo、期刊正式版等多個
版本，DOI 各異。系統以標題去重並保留**最早的公開日期** ——
優惠期要從最早公開日起算。

### 本模組的界線

只攤證據，**不做法律判斷**。是否真的構成新穎性障礙，需比對
申請專利範圍與論文的實際技術揭露內容，屬專利師專業範圍。

---

## 檢索詞抽取機制（最關鍵的一步）

**檢索詞錯了，整份報告就是錯的。** 這是本工具最容易出錯、也最需要
使用者確認的地方，因此介面在執行前會顯示「**檢索詞預檢**」讓您核對。

### 四種檢索詞，餵給四種資料庫

| 用途 | 詞的性質 | 範例（心震圖團隊） |
|------|---------|------------------|
| 裝置品名 | FDA 官方品名 | Ballistocardiography |
| 疾病狀態 | 臨床狀態 | Heart Failure |
| 中文品名 | TFDA 中文品名 | （需手動指定） |
| 技術關鍵詞 | 學術術語 | Ballistocardiography、bioimpedance |

> 為什麼要分開：裝置品名問「市面上有什麼產品」，技術詞問
> 「這個技術被研究過什麼」。實測用心震圖團隊的文件，若只用
> `wearable device` 查論文，會撈回「穿戴裝置與飲食習慣」
> 「藍牙穿戴裝置通訊」；改用 `ballistocardiography` 才撈到
> 「枕頭式 BCG 的 J 波偵測」—— 後者才可能構成新穎性障礙。

### 抽取優先序（信心由高到低）

1. **文件中的中英對照** ← 準確度最高
2. 手動指定
3. 內建對照表（含技術詞表）
4. 退路（**只產出中文，不當英文送出**）

### 最準的來源：您文件裡的中英對照

團隊寫計畫書時會自己標上英文專有名詞。系統會抓這個格式：

```
心衰竭 (Heart Failure, HF)        → Heart Failure (HF)
彈道心震圖 (Ballistocardiography, BCG) → Ballistocardiography (BCG)
```

**這是準確度最高的檢索詞來源** —— 那是團隊自己領域的正式術語，
比任何對照表或模型猜測都準確。如果您在計畫書裡附上英文全稱，
工具的準確度會大幅提升。

### 檢索品質自動驗證（怎麼知道查對了？）

工具會在檢索後**自動驗證結果是否真的相關**，並把判定結果顯示在最前面。

判定方式：用「文件中的中英對照」當標準，檢查撈回的文獻標題
是否含該技術概念。實測校準差距極大，訊號乾淨可用：

```
檢索詞                      文獻含核心概念的比例
ballistocardiography       94%
bladder scanner ultrasound 91%
incontinence pad           77%
wearable device             0%   ← 判定不通過
sensor                      0%   ← 判定不通過
中文原文直接送出             0%   ← 判定不通過
```

判定結果分四級：

- ✅ **通過** —— 技術詞命中率 ≥35%，可採用
- 🔸 **相關性偏低** —— 命中 15-35%，建議人工檢視
- ❌ **相關性不符** —— 命中 <15%，結果可能不是本產品，建議手動指定
- ❔ **無法自動驗證** —— **產品說明中沒有中英對照**，工具誠實說明
  無法判定，並附上樣本標題供人工檢視

> **實測結論：能不能自動驗證，取決於你的文件有沒有中英對照。**
> 有的話可自動判定（心衰竭案例：通過，技術命中率 79%）；
> 沒有的話工具不假裝驗證過，會明說並請你人工確認。

### 詞形變體檢索（避免漏掉三分之二的先前技術）

同一技術的詞形變體，資料庫撈回的結果**幾乎不重疊**：

```
ballistocardiography   84 篇
ballistocardiogram     61 篇
→ 共同只有 5 篇，聯集 244 篇
```

因此工具會自動以詞形變體補充檢索。實測效果：
心衰竭案例從 40 篇增至 **86 篇**（補得 46 篇）。

對「論文是否破壞專利新穎性」這種任務，漏掉一篇的代價極高，
所以這是必要的，不是最佳化。

### AI 協助抽取檢索詞（第一關）

產品說明往往是一整段中文計畫書文字。工具可以直接把長文交給語言模型，
抽出四組檢索詞：**裝置品名、疾病詞、中文品名、技術關鍵詞**。

在預檢畫面可勾選「用 AI 協助抽取檢索詞」。抽出的候選會**全部列出來**
讓你看到 AI 想到什麼。

> **重點：AI 只提候選，不決定答案。**
> 每個候選都會**實際送去檢索驗證**，查得到相關資料的才用。

### 最有價值的一步：從真實品名挑選

TFDA 的品名有固定的慣用語（例：「生理監視器」「超音波膀胱掃描儀」），
AI 憑空猜常常對不上。實測 AI 生成的詞：

```
「心臟血管監測器」→ 0 筆
「睡眠生理監測儀」→ 1 筆
```

所以工具改成：**先撈出 TFDA 資料庫裡實際存在的品名，讓 AI 從中挑選**。

膀胱餘尿案的效果：AI 挑出「**維塔康超音波膀胱掃描儀**」——
那正是最正確的既有品名。**AI 不可能編造出不存在的品名。**

### 台灣競品：兩層數字

只用最接近的品名檢索，實測常常只有 1 件。但育成輔導要回答的是
「這個品類有多擠」，所以工具給兩層：

```
精確層：最接近您產品的既有許可證（常只有 1 件）
同類層：整個品類的市場地景（例：膀胱超音波類 4 件、3 家廠商）
```

**判讀方式**：精確層少、同類層多 → 新切入點（有市場基礎但無直接對應品）；
兩層都少 → 全新品類，教育市場成本高。

### 一個已修正的設計錯誤

舊版在抽不到詞時，會**把中文原文直接送去查英文資料庫**。
實測結果是撈回「心力衰竭患者居家护理模式」這類完全無關的文獻，
而且報告看起來一切正常 —— 錯誤被掩蓋。

現已改為：抽不到就留空並明確警告，請使用者手動指定。
**寧可什麼都不查並告知，也不要產出誤導性結果。**

### 內建對照表刻意保持精簡

對照表只收「無歧義且在高頻使用」的詞。理由：這張表本身就是風險
來源 —— 它會把表外的詞放行成原文（災難），也會在表內詞有近義時
誤判。其餘一律靠文件中的中英對照或手動指定。

---

## 資料更新機制（重要）

各來源的更新行為**不同**，實測後分兩類（2026-09-22）：

### 即時查詢型 — 每次都用最新，不需任何動作

| 來源 | 官方資料日期（實測） | 落後 |
|------|-------------------|------|
| openFDA UDI | 2026-09-02 | 20 天 |
| openFDA 510(k) | 2026-09-14 | 8 天 |
| openFDA PMA | 2026-09-14 | 8 天 |
| openFDA 召回 | 2026-09-16 | 6 天 |
| openFDA 製造廠登記 | 2026-09-14 | 8 天 |
| PubMed | 即時 | 0 |
| FreePatentsOnline | 即時 | 0 |
| 健保署 | 即時 | 0 |
| Europe PMC／OpenAlex／Crossref／arXiv | 即時 | 0 |

> 落後 6-20 天是**官方開放資料本身的更新週期**（多為每月或每週），
> 不是工具延遲。工具每次執行都直接打官方 API。

### 快取下載型 — 不會自動更新，需重建

| 來源 | 資料日期 | 更新方式 |
|------|---------|---------|
| TFDA 許可證資料集 | 官方檔案 2026-09-17 | `python -m src.build_tfda_index --force` |
| UN Comtrade | 2022 年 | 年度資料，落後 3-4 年屬正常 |
| WHO / World Bank | 年度 | 台灣非會員國無資料 |

### TFDA 索引更新偵測

TFDA 是唯一「會過期且影響台灣競品準確度」的來源，已建自動偵測：

```bash
# 檢查官方是否已更新（不下載重建）
python -m src.build_tfda_index --check-update
```

原理：官方 ZIP 不提供 Last-Modified 標頭，因此下載後讀取
**ZIP 內檔案的建立時間戳**，與本地索引記錄的 `source_date` 比對。

索引內建 `meta` 表記錄官方來源日期。設計要點：**不可拿「建立時間」
當「來源日期」** —— 建立時間永遠晚於官方日期，會使本地看起來較新
而漏掉更新。

介面側邊欄的「資料新鮮度」面板可直接檢視，並有按鈕即時檢查。

**建議維護頻率**：TFDA 索引每季重建一次（或發現台灣競品結果
與實際取證狀況不符時）。

---

## 免責與使用界線

本系統是**早期探索與資料彙整輔助**，不是專業意見的替代品。

- 專利：非正式 FTO 檢索，正式檢索請委由專利師
- 法規：分類初判，最終以 TFDA / FDA 正式判定為準
- 競品：基於公開資料，不構成市占率或市場規模結論
- 疾病負擔：數字取自 PubMed 文獻摘要的原文引用，**非本工具推算**，
  惟摘要擷取可能混入不相關文獻，需人工核對原始文獻
- 給付：健保特材給付須核對最新支付標準
- **UN Comtrade 的數字是全部醫療器材的貿易額，不是本產品市場規模。**
  實測膀胱掃描儀案例：HS 9018 全類別加總 21.4 十億美元，
  該產品實際市場僅 8,274 萬美元，落差達 259 倍。
  本產品市場請用報告中的「由上而下市場推估」

每一筆外部資料都會標註查詢日期與來源。詳見 `docs/DISCLAIMER.md`。

---

## 與舊原型的關係

本專案是把 `/Volumes/MACRAID/專案/multiagent/` 裡已驗證可行的商品化三模組
抽出、重構為獨立專案。舊 repo 保留作為 Biodesign Identify 多代理人系統與研究紀錄。

抽出的原因：舊 repo 同時包含研究原型、多代理人實驗、報告產物與 venv，
不利於交給同仁使用與後續維護。本專案只保留可交付的部分。
